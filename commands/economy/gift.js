...
module.exports = {
	...
	async execute(interaction) {
		...
		const recipient = await global.userRecords.findOne({where: {userID: interaction.options.getUser("recipient").id}})
		...
		await interaction.editReply(`You have successfully gifted ${interaction.options.getInteger("amount")} credits to ${interaction.options.getUser("recipient")}.`)
	},
}